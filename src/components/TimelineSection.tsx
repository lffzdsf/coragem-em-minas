    ...
    <section id="timeline" className="py-24 md:py-32 bg-background section-padding">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-16 animate-on-scroll">
          <span className="text-gold text-sm font-semibold tracking-widest uppercase mb-4 block">Linha do tempo</span>
          ...
          <div className="space-y-12">
            {timelineItems.map((item, i) => {
              ...
              return (
                ...
                >
                  {/* Dot */}
                  <div className="absolute left-4 md:left-1/2 w-3 h-3 rounded-full bg-gold border-2 border-background -translate-x-1.5 md:-translate-x-1.5 mt-1.5 z-10" />

                  {/* Content */}
                  <div className={`ml-12 md:ml-0 md:w-1/2 ${isLeft ? "md:pr-12 md:text-right" : "md:pl-12"}`}>
                    <span className="font-heading text-gold font-bold text-lg md:text-xl block mb-2">{item.year}</span>
